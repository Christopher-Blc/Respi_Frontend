...
import {
  pagosStyles as styles,
  pagosKpiStyles as kpiStyles,
  pagosBadgeStyles as badgeStyles,
  pagosCardStyles as cardStyles,
  pagosChartStyles as chartStyles,
  pagosTableStyles as tableStyles,
} from '../../../../style/admin/pagos.styles';
    ...
    return (
      <View style={[tableStyles.row, { borderBottomColor: theme.borderSoft }]}>
        ...
        <View style={tableStyles.cell}>
          <Text
            style={[tableStyles.cellMain, { color: theme.textTitle }]}
            numberOfLines={1}
          >
            {userName}
          </Text>
          {!!court && (
            <Text
              style={[tableStyles.cellSub, { color: theme.textMuted }]}
              numberOfLines={1}
            >
              {court.name}
            </Text>
  ...
  return (
    ...
    >
      <View style={cardStyles.row}>
        ...
        <View style={{ flex: 1 }}>
          <Text
            style={[cardStyles.name, { color: theme.textTitle }]}
            numberOfLines={1}
          >
            {userName}
          </Text>
          {!!court && (
            <Text
              style={[cardStyles.sub, { color: theme.textMuted }]}
              numberOfLines={1}
            >
              {court.name} · {date}
            </Text>
        ...
        <Text style={[cardStyles.amount, { color: '#22c55e' }]}>
          {Number(p.amount).toFixed(2)} €
        </Text>
      </View>
      <View style={cardStyles.footer}>
        <StatusBadge status={p.payment_status} />
        <Text style={[cardStyles.method, { color: theme.textMuted }]}>
          {p.payment_method}
        </Text>
        {p.stripe_payment_intent_id && (
          <Text
            style={[cardStyles.pi, { color: theme.textSubtitle }]}
            numberOfLines={1}
          >
...
export default function PagosAdminScreen() {
  ...
  const pieData = useMemo(
    ...
    [stats.countPaid, stats.countRefunded, stats.countPending],
  );
    ...
    <View style={[styles.page, { backgroundColor: theme.background }]}>
      ...
      >
        ...
        <View style={[chartStyles.row, isWide && chartStyles.rowWide]}>
          ...
          >
            <Text style={[chartStyles.title, { color: theme.textTitle }]}>
              Ingresos mensuales
            </Text>
            <Text style={[chartStyles.sub, { color: theme.textSubtitle }]}>
              Últimos 6 meses · €
            </Text>
            {stats.totalRevenue === 0 ? (
              <Text style={[chartStyles.empty, { color: theme.textMuted }]}>
                Sin ingresos aún
              </Text>
            ) : (
              <ScrollView
                ...
                <PieChart
                  ...
                  centerLabelComponent={() => (
                    <View style={{ alignItems: 'center' }}>
                      ...
                      <Text style={{ color: theme.textSubtitle, fontSize: 9 }}>
                        total
                      </Text>
                    </View>
                  )}
        ...
        {Object.keys(stats.methodCounts).length > 0 && (
          ...
          >
            <Text style={[styles.methodTitle, { color: theme.textTitle }]}>
              Métodos de pago
            </Text>
              ...
              {Object.entries(stats.methodCounts)
                ...
                .map(([method, count]) => (
                  ...
                  >
                    ...
                    <Text
                      style={[styles.methodChipCount, { color: theme.primary }]}
                    >
                      {count}
                    </Text>
                  </View>
        ...
        <View style={styles.toolbar}>
          ...
          >
            ...
            <TextInput
              ...
              style={[
                styles.searchInput,
                { color: theme.textTitle, borderWidth: 0, outlineWidth: 0 },
              ]}
            />
            {!!search && (
              ...
              {(['cards', 'table'] as const).map((mode) => {
                ...
                return (
                  ...
                  >
                    <MaterialCommunityIcons
                      name={
                        mode === 'cards' ? 'view-grid-outline' : 'table-large'
                      }
                      size={18}
            ...
            <View style={{ minWidth: 860 }}>
              ...
              >
                {[
                  ...
                  <Text
                    key={h}
                    style={[tableStyles.headerCell, { color: theme.primary }]}
                  >
                    {h}
                  </Text>
                ))}
          ...
          <View style={isWide ? styles.cardsGrid : undefined}>
            {payments.map((p) => (
              <View
                ...
                style={
                  isWide
                    ? { flex: 1, minWidth: 300, maxWidth: '50%' }
                    : undefined
                }
              >
        ...
        {totalPages > 1 && (
          <View style={styles.pagination}>
            <TouchableOpacity
              style={[
                styles.pageBtn,
                { borderColor: page === 1 ? theme.borderSoft : theme.primary },
              ]}
            ...
            <Text style={[styles.pageInfo, { color: theme.textBody }]}>
              {page} / {totalPages}
            </Text>
            <TouchableOpacity
              style={[
                ...
                {
                  borderColor:
                    page === totalPages ? theme.borderSoft : theme.primary,
                },
              ]}
              onPress={() => setPage(page + 1)}
...
const styles = StyleSheet.create({
  ...
  searchInput: { flex: 1, fontSize: 14, outlineWidth: 0, borderWidth: 0 },
